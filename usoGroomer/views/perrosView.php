...
?>
    ...
    ?>
        ...
        <div class="bg-white p-4 rounded shadow mb-4 overflow-x-auto">
            ...
            <a href="http://localhost/Groomer-Lidia-PHP/usoGroomer/views/home.php?controller=perrosUso&action=showFormController&clienteDni=<?php if (isset($_GET['clienteDni'])) echo $_GET['clienteDni']; ?>"><button class="bg-green-500 text-white px-4 py-2 rounded m-4 dark:bg-blue-700">Insertar nuevo perro</button></a>
            <table class="min-w-full divide-y divide-gray-200 text-sm ">
                ...
                <tbody id="listaPerros" class="bg-white">
                    <?php

                    if (is_array($perrosCliente)) {
                        foreach ($perrosCliente as $perro) {
                            echo "<tr>";
                            echo "<td class='px-4 py-2'>{$perro['Dni_duenio']}</td>";
                            echo "<td class='px-4 py-2'>{$perro['Nombre']}</td>";
                            echo "<td class='px-4 py-2'>{$perro['Fecha_Nto']}</td>";
                            echo "<td class='px-4 py-2'>{$perro['Raza']}</td>";
                            echo "<td class='px-4 py-2'>{$perro['Peso']}</td>";
                            echo "<td class='px-4 py-2'>{$perro['Altura']}</td>";
                            echo "<td class='px-4 py-2'>{$perro['Observaciones']}</td>";
                            echo "<td class='px-4 py-2'>{$perro['Numero_Chip']}</td>";
                            echo "<td class='px-4 py-2'>{$perro['Sexo']}</td>";
